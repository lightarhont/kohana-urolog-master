...
class Controller_Chunks_Video extends Controller {
 
 //Рабочие переменные
 protected $imgcategories = '/public/usr/root/foto/imgcategories/';
 protected $icons = '/public/usr/root/foto/icons/';
 protected $albumslistlimit = '3';
 protected $categorylistlimit = '10';
 ...
 public function action_defcategory() {
  ...
  $fotos = Model::factory('Showcatvideo')->get_deffotos($this->categorylistlimit);
 ...
 public function action_idcategory() {
  ...
  if ($catid == 0) {
   ...
   $fotos = Model::factory('Showcatvideo')->get_catidfotos($catid, $this->categorylistlimit);
  }
 ...
 public function action_pagescategory() {
  ...
  $fotos = Model::factory('Showcatvideo')->get_pages_fotos($catid, $offset, $this->categorylistlimit);
  $this->paginationfotos($fotos, $total);
  //$this->response->body(var_dump($fotos));
 }
  
 protected function deffotoscount() {
  $counts = Model::factory('Showcatvideo')->get_deffotos_count();
  $total = 0;
 ...
 protected function category($fotos, $total) {
  ...
  <div class="thistitle">'.$fotos[0]->cattitle.'</div>
 ...
 <div id="pagesloading" class="images">';
  ...
  foreach ($fotos as $foto) {
   $album .= '<a target="_blank" href="/showvideo/id-'.$foto->id.'.html" class="icn"><img src="'.$this->icons;
   $album .= $foto->img;
   $album .= '" /></a>';
  }
  $album .= '</div>';
