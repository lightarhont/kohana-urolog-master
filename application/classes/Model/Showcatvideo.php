...
class Model_Showcatvideo extends Model {
 
 protected $_tablecatfotos = 'cat_videos';
 protected $_tablefotos    = 'videos';
 
 public function get_deffotos($limit=10) {
 
 $sql = "SELECT `".$this->_tablefotos."`.*, `".$this->_tablecatfotos."`.`description` AS `catdesc`, `".$this->_tablecatfotos."`.`title` AS `cattitle` FROM `".$this->_tablefotos."` JOIN `".$this->_tablecatfotos."` ON (`".$this->_tablefotos."`.`catid` = `".$this->_tablecatfotos."`.`id`) WHERE `".$this->_tablecatfotos."`.`catdefault` = :catdefault LIMIT ".$limit;
 ...
 $sql = "SELECT `".$this->_tablefotos."`.*, `".$this->_tablecatfotos."`.`description` AS `catdesc`, `".$this->_tablecatfotos."`.`title` AS `cattitle` FROM `".$this->_tablefotos."` JOIN `".$this->_tablecatfotos."` ON (`".$this->_tablefotos."`.`catid` = `".$this->_tablecatfotos."`.`id`) WHERE `".$this->_tablefotos."`.`catid` = :catid LIMIT ".$limit;
 $query = DB::query(Database::SELECT, $sql);
 $query->param(':catid', $catid);
